...
%            title(sprintf('HAS Bead R = %.2f(um)',bRadius(fileId)));
            ...
            viscircles(centroid,coat.bRadius/pxlSize,'LineStyle','-','LineWidth',5,'EdgeColor','b');
            viscircles(centroid,cRadius(fileId)/pxlSize,'LineStyle','-','LineWidth',3,'EdgeColor','r');
            hold off
            subplot(2,2,3);
            %plot profile
            hold on
            ...
            for pId = 14:14%:length(coat.labelArray)
                fittingResult = [];
                ...
                x = x';
                x = x*pxlSize;
                plot(x-x(peakId),y,'LineWidth',3);
                
               % plot(x(peakId),y(peakId),'*','MarkerSize',3);
               % plot(x(endId),y(endId),'*','MarkerSize',3);
                %f = fit(x(peakId:endId),y(peakId:endId),'exp1');
            ...
            return
            coat.fittingResult = fittingResult;
            subplot(2,2,4);
            hold on
...
save('coatCellFITWT.mat','coatCell');
